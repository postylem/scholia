...
- Quarto project `.qmd` files (websites/books with a non-default `output-dir`) are supported in `scholia view`: the renderer detects the project via `quarto inspect`, reads the rendered HTML from the project's `output-dir`, and serves assets through the `/quarto-site/` route. Site chrome (navbar/sidebar) is kept; sibling pages opened from the navbar are served as static project HTML without the scholia sidebar.
